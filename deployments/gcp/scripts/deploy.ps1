...
gcloud compute ssh $Instance --project $ProjectId --zone $Zone --command "cd $remote && rm -rf app && mkdir app && tar -xf /tmp/pulsequeue-deploy.tar -C app && cp .env app/.env && cd app && docker compose -f deployments/docker-compose.yml --env-file .env up -d --build"
